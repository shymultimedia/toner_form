...
$my_global_var_counter = 1; //GLOBAL VARIABLE TO COUNT NEW PRINTER TONER ROWS -S.B., 03-04-16

var global_tempQuant = 0; //GLOBAL VARIABLE TO COUNT QUANTITY FIELD -S.B., 03-04-16

$global_sNo = 1;  //GLOBAL VARIABLE TO COUNT TONER REQUEST ROWS -S.B., 03-10-16



//ONCHANGE FUNCTIONS FOR EXTRA DROPDOWNS**********************************************
function reset_sNos(){

var temp = -1;
	$(".textbox_small_disabled").each(function(i, item){
		temp = temp + 1;
//		$(this).val(temp);
//alert(temp);
		$(this).attr("value", temp);  //USING THIS ONE -S.B., 03-10-16															
//		$("#ordering_code_" + rowNumber).attr("value", $orderCodeVal);  //USING THIS ONE -S.B., 03-10-16															
...
function updateQuantity(){
	...
	global_tempQuant = 0;//parseInt("0",10);
	...
	//alert($(this).val());
	 ...
	 //if(isNan( parseInt($(this).val()) ) == false) {  //MAY BE ABLE TO SKIP CHECK -S.B., 03-08-16
//	 	global_tempQuant = global_tempQuant + parseInt($(this).val());
	 	global_tempQuant = global_tempQuant + temp;   //parseInt($(this).val(),10);
	 //}	 //END MAY BE ABLE TO SKIP CHECK -S.B., 03-08-16



	});  //END .EACH

	//$('.totalQuant').val(global_tempQuant);	
//	$('.totalQuant').attr("value",global_tempQuant);	
	$('.totalQuant').val(global_tempQuant);
...
//alert(value);
global_tempQuant = 0;
//	global_tempQuant += value;//parseInt(value);

	$(".quantity").each(function(i, item){
		// var temp = $(this).val();
		var temp = +($(this).val()) || 0;

//		global_tempQuant = global_tempQuant +  parseInt(temp);//$(this).val();
		global_tempQuant = global_tempQuant +  temp;//$(this).val();
			});
	$('.totalQuant').val(global_tempQuant);

}

function dropDownLogic_toner(that){

		$myDropdown = $(that);
///		$myDropdown = $(this);
		$orderCodeVal = $myDropdown.val();
		var rowNumber = findRowNumber_toner($myDropdown.attr('id'));  //GETS NUMBER OF ROW
//		alert(rowNumber);
//		var orderCodeName = new String("#ordering_code_" + rowNumber + "select"); //PRIOR -S.B., 03-10-16
		var orderCodeName = new String("#ordering_code_" + rowNumber);

			//$(orderCodeName).attr("value", $orderCodeVal);  //PRIOR -S.B., 03-10-16

			$("#ordering_code_" + rowNumber).attr("value", $orderCodeVal);  //USING THIS ONE -S.B., 03-10-16													
}

function dropDownLogic(that){ 
//	alert("DROPDOWN TEST");
		var dropDownThis = $(that);//$(this);
		$jThis = $(that);		
		//$jThis = $(this);
        $.ajax({
          type: "GET",
          url: "ajax/database_process.php",
//          data: {"printer": $('.dropdown_printer_model').val()  },
		  data: {"printer": dropDownThis.val()  },
          datatype: "json", //"text"                                                          

          success: function(data){


          	//alert("SUCCESS:" + dropDownThis.val()); 20
          	//alert("SUCCESS:" + data);
          	var myArray = JSON.parse(data);  //****
					 //$.each(myArray, function(i, item) {
 					//var myArray = JSON.parse(data);
					var rowNumber = findRowNumber($(dropDownThis).attr('id'));
			        var dTonerName = "#dropdown_toner_" + rowNumber;

			        $(dTonerName).empty(); //EMPTY DROPDOWN, GETTING READY FOR NEWER DATABASE SELECTIONS -S.B., 03-04-16
					 $.each(myArray, function(i, item) {  //*****
				 	//$.each(data, function(i, item) {
				 		//alert(item.OrderingCode);
				 		//if(item.PID == dropDownThisValue ){}
				 		var pid_string = new String(item.PID);
				 		var dropdownitemPID_String = new String(dropDownThis.val());
				 		if(pid_string.trim() == dropdownitemPID_String.trim() ){
				 			//alert("a match: "+item.OrderingCode+ item.Supply);
//				 		if(if item.PID = dropDownThis.val() ){}
				 		//var newOption = $('<option value=\"'+item.OrderingCode+'\">'+ item.Supply +'</option>'); //LAST ONE USED -S.B., 03-07-16
						var newOption = "<option value=\""+item.OrderingCode+"\">"+ item.Supply +"</option>";				 		
			                //$jThis.closest('.dropdown_toner select').append( new Option(item.Supply,item.OrderingCode) );
			                //$jThis.closest('.dropdown_toner select').append(newOption);

			                //alert(dTonerName);
			                //$(dTonerName).append(newOption);  //LAST ONE USED -S.B., 03-07-16
//							$("#dropdown_toner_" + rowNumber).append("<option value=\""+item.OrderingCode+"\">"+ item.Supply +"</option>"); LAST ONE TOO			

							$("#dropdown_toner_" + rowNumber).append( new Option(item.Supply,item.OrderingCode) );				                
							//$('.ordering_code').val(item.OrderingCode); //MAY NEED TO USE FOR ORDERING CODE -S.B., 03-04-16		                

			                //$('#dropdown_toner_0').append(newOption);	
							$orderCodeName = "#ordering_code_" + rowNumber;
							$("#ordering_code_" + rowNumber).attr("value", item.OrderingCode);  //USING THIS ONE													                
			                }//END IF -S.B., 03-04-16		                
				 		}
          				);  //end EACH

					$("#dropdown_toner_" + rowNumber).attr("disabled", false);	//selectmenu("enable");		        					 
...
//END ONCHANGE FUNCTIONS FOR EXTRA DROPDOWNS*****************************************

function newPrinterData(printerModel){

			               // alert(printerModel);

				                $.getJSON('file4.json', function(data) {	

									$.each(data, function(i, item) {

						                //$(printerModel).append( new Option(item.Model,item.PID) );				                

					 		var newOption = $('<option value=\"'+ item.PID +'\">'+ item.Model +'</option>');
			                //$jThis.closest('.dropdown_toner select').append( new Option(item.Supply,item.OrderingCode) );
			                //$jThis.closest('.dropdown_toner select').append(newOption);


			                $(printerModel).append(newOption);
...
function findRowNumber( pModelName ){
    var x_string = new String(pModelName);//tr(pModelName);
	//"dropdown_toner"
	var str = x_string.replace("dropdown_printer_model_","");
	str = str.trim();
	
	return str;
}

function findRowNumber_toner( pModelName ){
    var x_string = new String(pModelName);//tr(pModelName);
	//"dropdown_toner"
	var str = x_string.replace("dropdown_toner_","");
	str = str.trim();
...
function clickRow(){
	$my_global_var_counter++; //increment for new rod id value

	$temp_model = "dropdown_printer_model_" + $my_global_var_counter;
	$temp_model_id = "#dropdown_printer_model_" + $my_global_var_counter;	

	$temp_toner =  "dropdown_toner_" + $my_global_var_counter;
	$temp_ordering_code =  "ordering_code_" + $my_global_var_counter;	
...
//		newRow = "<tr><td class=\"serial_no\"><span class = \"sNo_count\">&nbsp;</span></td><td class=\"dropdown_printer_model\"><select class=\"dropdown_printer_model\" name=\"PrinterModel\"><option value=\"test1\">test1</option><option value=\"test2\">test2</option></select></td><td class=\"dropdown_toner\"><select class=\"dropdown_toner\" name=\"Toner\"><option value=\"test1\">test1</option><option value=\"test2\">test2</option></select></td><td class=\"ordering_code\" ><input type=\"textbox\" class=\"ordering_code\" name=\"OrderingCode\"></td><td class=\"quantity\"><input type=\"textbox\" class=\"quantity\" name=\"Quantity\"></td><td class=\"add\"><img src=\"images/icon-add.png\" class=\"addRowButton\"></td></tr>";
				...
				newRow = "<tr class=\"addlRow\"><td class=\"serial_no\"><input type = \"textbox\" name = \"SNo[]\" class=\"textbox_small_disabled\"value=\""+$global_sNo+"\"> </td><td class=\"dropdown_printer_model\"><select  id=\"" +$temp_model +"\" class=\"dropdown_printer_model\" name=\"PrinterModel[]\" onChange=\"dropDownLogic(this);\"><option value=\"\">&nbsp;</option><option value=\"\">&nbsp;</option></select></td><td class=\"dropdown_toner\"><select id=\"" + $temp_toner +"\"  class=\"dropdown_toner\" name=\"Toner[]\" disabled=\"true\" onChange=\"dropDownLogic_toner(this);\"><option value=\"\">&nbsp;</option><option value=\"\">&nbsp;</option></select></td><td class=\"ordering_code\" ><input type=\"textbox\" class=\"ordering_code\" id=\""+$temp_ordering_code+"\" name=\"OrderingCode[]\" disabled=\"true\"></td><td class=\"quantity\"><input type=\"textbox\" id=\"quantity_"+$my_global_var_counter+" \" class=\"quantity\" name=\"Quantity[]\" value=\"1\" onchange=\"changeQuantity(this.value);\"></td><td class=\"add\"><a class=\"add_row_link\" href=\"#\"><img src=\"images/icon-add.png\" class=\"addRowButton\"></a></td></tr>";
...
//*INSERT PRINTER MODEL  DATA ******************************************
                ...
                $("#dropdown_printer_model_" + $my_global_var_counter).append(newOption);

											 });		                    	
	                    }  

                );//);  //end getJSON
//***END INSERT PRINTER MODEL DATA******************************************		
				...
				updateQuantity();
					...
					//DECREMENT TOTAL QUANTITY FIELD -S.B., 03-04-16
...
$(document).ready(function(){
		...
		ko.applyBindings(printerDiv, document.getElementById('printerDiv'));			
		...
		// CODE TO POPULATE PRINTER MODEL ******************************************
		    $.getJSON('file4.json', function(data) {	

					$.each(data, function(i, item) {
		//						                $('.dropdown_printer_model select').append( new Option(item.Model,item.PID) );				                
		                $('#dropdown_printer_model_0').append( new Option(item.Model,item.PID) );				                
										 });		                    	
		            }  
		...
		//END CODE TO POPULATE PRINTER MODEL ******************************************
...
//     "Please select a value");
		...
		var validator = $('#tonerForm').validate(
...
// //																aFunction: true
											 	 ...
											 	 messages: {
													  		'Requestor':
													  		{
													  		    required:  "<span style=\"color: red;\">Please enter the Requestor name</span>"
													  		},
													  		'email':
													  		{
													  			required: "<span style=\"color: red;\">Please enter a valid email address</span>"
...
function isValid(){
	//$(".quantity").each(function(i, item){
		var varResult = new Boolean(true);		
...
//QUANTITY CHECK
		...
		$('.quantity').each(function(index, value){
			...
			//if (value == '0'){	
				$(".errormsg").html("<p><span style=\"color: red;\">Please select Quantity</span></p>");
				varResult =  false;
		...
		if(varResult == false){return false;}
...
//LOCATION
		...
		$('.Location').each(function(index, value){
			...
			//if ($(this).attr("value") == "0"){	
			//if (value == '0'){	
				$(".errormsg").html("<p><span style=\"color: red;\">Please select Location</span></p>");
				varResult =  false;
...
//END LOCATION
		...
		$('.dropdown_printer_model').each(function(index, value){

			if ($(this).val() == "0"){
			//if ($(this).attr("value") == "0"){	
			//if (value == '0'){	
				$(".errormsg").html("<p><span style=\"color: red;\">Please select Printer Model</span></p>");
				varResult =  false;
		...
		$('.dropdown_toner').each(function(index, value){

//			if ($(this).attr("value") == "0"){	
					if ($(this).val() == "0"){
//				if (value == '0'){	
						$(".errormsg").html("<p><span style=\"color: red;\">Please select Toner</span></p>");
						varResult =  false;
...
$('form').on('submit', function(event) {
		...
		if(formValid == true && $('#tonerForm').valid() == true){
			//alert("formvalid");
						 //$.each(myArray, function(i, item) {  //*****		
			//(for each printer model, if value = 0, then put out "Please select Printer Model".){}


		$.ajax('ajax/processform.php', {
		type: 'POST',
		data: $('form').serialize(),	
		success: function(result){

//			alert("form processed:" + result)
			$('form')[0].reset();

			//REMOVE ADDITIONAL ROWS
			$(".addlRow").each(function(index){
				$(this).remove();

			})  //END .EACH
			//alert(result);  //WILL ALERT VALIDATION ISSUE

			//ENSURES RESET OF INITIAL PRINTER SUPPLY ROW -S.B., 03-08-16
				$("#dropdown_printer_model_0").attr("value", 0);  //USING THIS ONE
	//			$("#dropdown_toner_0").attr("value", 0);  //USING THIS ONE
				//$("#dropdown_toner_0").empty();  //USING THIS ONE (DIDN'T WORK)
				$("#dropdown_toner_0 > option").remove();  //USING THIS ONE			
				$("#ordering_code_0").attr("value", " ");  //USING THIS ONE		
					
				$("#first_add_row_img").attr('src', 'images/icon-add.png') ;

//				$(".errormsg").html("<p><span style=\"color: #1c8c7f;  font-weight:bold;\">Your request has been submitted.</span></p>");
				$(".errormsg").html("<p><span style=\"color: #1c8c7f;  font-weight:bold;\">Your request has been submitted.</span></p>").fadeIn(1500).delay(3500).fadeOut(500);

			//$("#dropdown_printer_model_0").val(0);
			//$("#dropdown_toner_0").val(0);
			//$("#ordering_code_0").val(0);		
		...
		error: function(request,errorType,errorMessage){
					...
					alert("error:"+ request + errorType + errorMessage);
...
});  //END $('form').on('submit', function(event)

//QUANTITY FIELD UPDATE (WHENEVER QUANTITY IS CHANGED)
$(".quantity").change(function(){updateQuantity();});



		$('.dropdown_toner select').change( function(){
		$myDropdown = $(this);
		$orderCodeVal = $myDropdown.val();
		var rowNumber = findRowNumber_toner($myDropdown.attr('id'));  //GETS NUMBER OF ROW
//		alert(rowNumber);
...
$('.dropdown_printer_model select').change( function(){
//	alert("DROPDOWN TEST00");
...
//          	alert("SUCCESS:" + dropDownThisValue.val());
          	//alert("SUCCESS:" + data);
          	var myArray = JSON.parse(data);  //****
					 //$.each(myArray, function(i, item) {
 					//var myArray = JSON.parse(data);
...
//***********************************
					...
					//	    var orderCodeName = "#ordering_code_" + rowNumber + "select";
							//$(orderCodeName).val(item.OrderingCode);							
							//$(orderCodeName).attr("value", item.OrderingCode);  //USING THIS ONE							


//							$("#ordering_code_0").attr("value", "ordering_code_0");  //PROGRESS!!!  ***USING THIS ONE														
//							$(orderCodeName).attr("value", "ordering_code_x");  //USING THIS ONE   														
							//$("#ordering_code_" + rowNumber).attr("value", "ordering_code_x");  //USING THIS ONE	XOXOXOXOXOXOXXOO													
...
$('.Location').change( function(){  //WORKS PERFECTLY!  ***** -S.B., 03-03-16

        $.ajax({
          type: "GET",
          url: "ajax/database_process_location.php",
          ...
          var myArray = JSON.parse(data);
					 ...
					 $.each(myArray, function(i, item) {
			            $('.phone_textbox').val(item.Phone);
			            $('.phone_textbox').css('background-color', '#ffffff'); 


					 });
          },//END SUCCESS
          error: function(request,errorType,errorMessage){alert("error"+ errorMessage);}
...
//$.getJSON("ajax/locationdata.json", {"Location": $('.Location').val()  },function(data) {
	// $.getJSON("ajax/locationdata.json", 
	// 			function(data) {
	// 	                            response($.map(data, function(item, index) {
	// 			                         $('.phone_textbox').value = item.Phone;
	// 			                         alert(item.Phone);
