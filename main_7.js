...
$global_tempQuant = 0; //GLOBAL VARIABLE TO COUNT QUANTITY FIELD -S.B., 03-04-16
...
function changeQuantity(value){
	...
	$global_tempQuant += parseInt(value);
...
//		newRow = "<tr><td class=\"serial_no\"><span class = \"sNo_count\">&nbsp;</span></td><td class=\"dropdown_printer_model\"><select class=\"dropdown_printer_model\" name=\"PrinterModel\"><option value=\"test1\">test1</option><option value=\"test2\">test2</option></select></td><td class=\"dropdown_toner\"><select class=\"dropdown_toner\" name=\"Toner\"><option value=\"test1\">test1</option><option value=\"test2\">test2</option></select></td><td class=\"ordering_code\" ><input type=\"textbox\" class=\"ordering_code\" name=\"OrderingCode\"></td><td class=\"quantity\"><input type=\"textbox\" class=\"quantity\" name=\"Quantity\"></td><td class=\"add\"><img src=\"images/icon-add.png\" class=\"addRowButton\"></td></tr>";
		newRow = "<tr><td class=\"serial_no\"><span class = \"sNo_count\">&nbsp;</span></td><td class=\"dropdown_printer_model\"><select  id=\"" +$temp_model +"\" class=\"dropdown_printer_model\" name=\"PrinterModel\" onChange=\"dropDownLogic(this);\"><option value=\"\">&nbsp;</option><option value=\"\">&nbsp;</option></select></td><td class=\"dropdown_toner\"><select id=\"" + $temp_toner +"\"  class=\"dropdown_toner\" name=\"Toner\" disabled=\"true\"><option value=\"\">&nbsp;</option><option value=\"\">&nbsp;</option></select></td><td class=\"ordering_code\" ><input type=\"textbox\" class=\"ordering_code\" id=\""+$temp_ordering_code+"\" name=\"OrderingCode\" disabled=\"true\"></td><td class=\"quantity\"><input type=\"textbox\" id=\"quantity_"+$my_global_var_counter+" \" class=\"quantity\" name=\"Quantity\" onchange=\"changeQuantity(this.value);\"></td><td class=\"add\"><a class=\"add_row_link\" href=\"#\"><img src=\"images/icon-add.png\" class=\"addRowButton\"></a></td></tr>";
		...
		//INCREMENT TOTAL QUANTITY FIELD -S.B., 03-04-16
				// $global_tempQuant = $global_tempQuant + 1;
				// $('.totalQuant').val($global_tempQuant);

//*INSERT PRINTER TONER  DATA (IRRELEVANT?)********************************************
		//newPrinterData($temp_model_id);
...
//		$(this).closest('row_toner_detail').remove();
		var temp = parseInt($(this).parent().find('.Quantity').val());
		alert(temp);
		...
		if($('.totalQuant').val() > 0){
//			$global_tempQuant = $global_tempQuant - 1;
			$global_tempQuant = $global_tempQuant - temp;
			//$('.totalQuant').val($global_tempQuant);
		}
	}
...
// $('.dropdown_printer_model').change( function(){
// 		$.ajax({
// 		  type: "GET",
// 		  url: "ajax/database_process.php",
// //		  data: "name=John&location=Boston",
// 		  data: {"printer": $('.dropdown_printer_model').val()  },
//   //data: {printer: this.el.attr('value')}, 							  							  

// 		  success: function(msg){
// 		    console.log( "Ajax Data Saved: " + $('.dropdown_printer_model').val() + msg );
// 		  }
// 		}); 	
...
//             source: function(request, response) {
//                 $.getJSON('ajax/tonerdata.json', request, function(data) {	
// //                $.getJSON('ajax/process.php', request, function(data) {	
// 					//console.log(data);
//                     //var selectOnlyOption = data.length <= 1;
//                     response($.map(data, function(item, index) {
...
type: 'POST',
data: $('form').serialize(),	
success: function(result){alert("form saved!")}
});
});
...
// }	
		...
		var rowNumber = findRowNumber($myDropdown.attr('id'));  //GETS NUMBER OF ROW
//		alert(rowNumber);
		var orderCodeName = new String("#ordering_code_" + rowNumber + "select");
//	    var orderCodeName = "#ordering_code_" + rowNumber + "select";

			//$(orderCodeName).val($orderCodeVal);
			

			$(orderCodeName).attr("value", $orderCodeVal);  //USING THIS ONE
			//$('.ordering_code_0').attr("value", "TEST 03-04-16");
